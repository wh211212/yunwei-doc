...
NGINX_STATS=$(sudo $DOCKER_BIN exec -i $CONTAINER_NAME $CURL_BIN $URL 2> /dev/null)
NGINX_VERSION=$(sudo $DOCKER_BIN exec -i $CONTAINER_NAME $NGINX_BIN -v 2>&1 | awk -F '/' '{print $2}')
