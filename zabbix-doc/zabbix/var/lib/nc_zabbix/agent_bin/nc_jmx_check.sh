...
JAR="$CURRENT_DIR/../bin/cmdline-jmxclient-0.10.3.jar"
JAVA=$(ps -e -o command | grep -v grep | grep -E "java.*jmxremote.port" | awk '{ print $1 }' | head -n1) # Get the Java bin path
...
JMX_OUT=$("$JAVA" -jar "$JAR" "$JMX_USER":"$JMX_PASS" "$JMX_HOST":"$JMX_PORT" "$BEAN_NAME" "$PROPERTY" 2>&1 | sed -ne '1 s/\([^\ ]* \)\{4\}//; 1p; 1!p')
